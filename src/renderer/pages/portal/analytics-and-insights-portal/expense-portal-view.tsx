...
import { cn, convertToTitleCase, monthNumberToString } from "src/lib/utils";
import { MonthlyCategorizedExpenditureMetricsView } from "./monthly-categorized-expenditure";
import { columns } from "src/components/data-table/data-column";
import { DataTable } from "src/components/data-table/data-table";
import { HistoricalAccountBalanceChart } from "src/components/charts-and-graphs/historical-account-balance-chart";
...
import { ExpenseCategoryDialog } from "src/components/category-expense-modal";
...
const ExpenseViewContext = createContext<ExpenseMetrics[] | undefined>(
  undefined
);

export type ExpenseViewProps<T extends ExpenseMetrics[]> = {
...
export type ExpenseViewState<T extends ExpenseMetrics[]> = {
 ...
 */
export class ExpenseView<T extends ExpenseMetrics[]> extends Component<
  ExpenseViewProps<T>,
  ExpenseViewState<T>
> {
  ...
  constructor(props: ExpenseViewProps<T>) {
    ...
    this.getListByCategory = this.getListByCategory.bind(this);
  ...
  getListByCategory(category: string): ExpenseMetrics[] {
    ...
    const filteredByCategory = data.filter(
      (expense) => expense.personalFinanceCategoryPrimary === category
    );
  ...
  render(): ReactNode {
    ...
    return (
      <ExpenseViewContext.Provider value={data}>
        <div
...
export const ExpenseAnalytics: React.FC<{
  ...
  React.useEffect(() => {
    ...
    for (let i = 1; i < months.length; i++) {
      ...
      const currentIncome = monthlyExpenses[currentMonth];
      const previousIncome = monthlyExpenses[previousMonth];
        ...
        <TabsContent value="statistics">
          <div className="flex flex-col gap-2 p-[5%]">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              ...
              />
              {monthWithHighestTransactions.toString().length === 6 && (
                <SimpleStatsCard
                  title={"Month with Highest Transactions"}
                  metric={monthNumberToString(monthWithHighestTransactions)}
          ...
          <div className="flex flex-col gap-2 p-[5%]">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              ...
              <SimpleStatsCard
                ...
                subtext="The category with the highest expenditure"
              />
              {monthWithLowestExpenditure.toString().length === 6 && (
                <SimpleStatsCard
                  title={"Month with Lowest Transactions"}
                  metric={monthNumberToString(monthWithLowestExpenditure)}
                  subtext="The month with the lowest number of transactions"
                />
              )}
              <SimpleStatsCard
