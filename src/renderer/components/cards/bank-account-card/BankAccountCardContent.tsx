...
import { BankAccountClass } from "@solomon-ai/component-library";
...
const BankAccountCardContent: React.FC<IBankAccountCardContent> = (props) => {
  ...
  const goals = bankAccountClassInstance.getGoals();
      ...
      {milestones.map((milestone) => (
        <CommandItem key={milestone.id}>
          <div className="flex justify-between gap-4 px-2">
            ...
            <Label className="px-3 text-xs font-bold border rounded-2xl">
              {" "}
        ...
        <CollapsibleContent className="space-y-2">
          {bankAccount.pockets.slice(1).map((pocket) => {
            return (
              <div
                key={pocket.id}
                className="px-4 py-2 font-mono text-sm border rounded-md shadow-sm"
              >
