...
/** @type {React.Context<PlaidAccountTransaction[]>} */
const TransactionAnalyticsByMonthContext = createContext<
...
> {
  ...
  private categorizeTransactions(
    ...
    for (const transaction of transactions) {
      ...
      const currentCategory = categoriesMap.get(categoryName) || {
        ...
        numTransactions: 0,
        amount: 0,
        transactions: [] as PlaidAccountTransaction[],
      };
  ...
  private groupByMonth(
    ...
    const monthlyTransactions = new Map<string, PlaidAccountTransaction[]>();
    ...
    for (const [monthYear, monthTrans] of Array.from(
        ...
        <Card className={cn("w-full", className)}>
          <div className="flex flex-col gap-2 p-4">
            {/* <Tabs defaultValue={"txnbymonth"} className="min-w-[400px]">
              <TabsList className="m-3 grid grid-cols-2 font-bold rounded-2xl bg-gray-300 w-[400px]">
              ...
              <TabsContent value="txnbymonth" className="mt-2 w-[100%]">
...
const TransactionsCategorizePieChart: React.FC<{
  ...
  return (
    <TremorCard className="max-w-4xl mx-auto mt-2">
...
const TransactionCategories: React.FC<{
  ...
  const TransactionContent: React.FC<{ item: MonthTransactions }> = ({
    ...
    const getAlltransactionForTheMonthAcrossAllCategories = () => {
      const allTransactions: PlaidAccountTransaction[] = [];
      ...
      return (
        <div className="mt-4 flex flex-wrap gap-4">
          {item.data.map((monthlyTransactionData) => (
            <Card key={monthlyTransactionData.name} className="flex flex-1 p-2">
              <div className="flex flex-1 space-x-4 truncate">
                <Icon
                  variant="light"
                  icon={RocketIcon}
                  size="md"
                ...
                <div className="truncate">
                  ...
                  <p className="truncate">{`${monthlyTransactionData.numTransactions} transactions`}</p>
                </div>
              </div>
              <div>
                <p>{`$${monthlyTransactionData.amount}`}</p>
              </div>
            </Card>
          ))}
        </div>
    ...
    return (
      <div className="mt-2 w-[100%]">
        ...
        >
          <TabsList className="grid w-full grid-cols-2 font-bold rounded-2xl max-w-[400px] bg-gray-300 m-3">
  ...
  return (
    <Tabs defaultValue={monthlyTransactions.at(0)?.name}>
      <div className="flex flew-1 gap-[5%]">
        <div>
          <TabsList className="flex flex-col gap-2 p-3 border rounded-2xl">
...
const ProgressBarHelperComponent: React.FC<{
  ...
  return (
    <ListItem key={txn.transactionName}>
